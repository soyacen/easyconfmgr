...
func TestTrieTreeValuer(t *testing.T) {
	...
	key6Val, err := valuer.GetFloat64("key6")
	if err != nil {
		t.Fatal(err)
	}
	if key6Val != newKey6 {
	...
	key3Val, err := valuer.GetBool("key3")
	if err != nil {
		t.Fatal(err)
	}
	if key3Val != newKey3 {
	...
	subconf2key2Val, err := valuer.GetStringSlice("subconf2.key2")
	if err != nil {
		t.Fatal(err)
	}
	...
	subconfVal, err := valuer.GetStringMap("subconf")
	if err != nil {
		t.Fatal(err)
	}
