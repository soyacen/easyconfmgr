...
import (
	...
	"github.com/soyacen/easyconfmgr/parser"
...
var configFile = "/tmp/config.yaml"
...
func TestManager(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, config.Bool, boolVal)

	intVal, err := manager.GetInt("int")
	assert.Nil(t, err)
	assert.Equal(t, config.Int, intVal)

	int32Val, err := manager.GetInt32("int_32")
	assert.Nil(t, err)
	assert.Equal(t, config.Int32, int32Val)

	int64Val, err := manager.GetInt64("int_64")
	assert.Nil(t, err)
	assert.Equal(t, config.Int64, int64Val)

	uintVal, err := manager.GetUint("u_int")
	assert.Nil(t, err)
	assert.Equal(t, config.UInt, uintVal)

	uint32Val, err := manager.GetUint32("u_int_32")
	assert.Nil(t, err)
	assert.Equal(t, config.UInt32, uint32Val)

	uint64Val, err := manager.GetUint64("u_int_64")
	assert.Nil(t, err)
	assert.Equal(t, config.UInt64, uint64Val)

	float64Val, err := manager.GetFloat64("float_64")
	assert.Nil(t, err)
	assert.Equal(t, config.Float64, float64Val)

	timeVal, err := manager.GetTime("time")
	assert.Nil(t, err)
	assert.Equal(t, config.Time.UnixNano(), timeVal.UnixNano())

	durationVal, err := manager.GetDuration("duration")
	assert.Nil(t, err)
	assert.Equal(t, config.Duration, durationVal)

	intSliceVal, err := manager.GetIntSlice("int_slice")
	assert.Nil(t, err)
	assert.Equal(t, config.IntSlice, intSliceVal)

	stringSliceVal, err := manager.GetStringSlice("string_slice")
	assert.Nil(t, err)
	assert.Equal(t, config.StringSlice, stringSliceVal)

	StringMapStringVal, err := manager.GetStringMapString("string_map_string")
	assert.Nil(t, err)
	assert.Equal(t, config.StringMapString, StringMapStringVal)

	StringMapStringSliceVal, err := manager.GetStringMapStringSlice("string_map_string_slice")
	assert.Nil(t, err)
	assert.Equal(t, config.StringMapStringSlice, StringMapStringSliceVal)
	...
	assert.Nil(t, err)
	assert.Equal(t, config.Map.String, mapString)

	mapBool, err := manager.GetBool("map.bool")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.Bool, mapBool)

	mapInt, err := manager.GetInt("map.int")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.Int, mapInt)

	mapInt32, err := manager.GetInt32("map.int_32")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.Int32, mapInt32)

	mapInt64, err := manager.GetInt64("map.int_64")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.Int64, mapInt64)

	mapUInt, err := manager.GetUint("map.u_int")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.UInt, mapUInt)

	mapUInt32, err := manager.GetUint32("map.u_int_32")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.UInt32, mapUInt32)

	mapUInt64, err := manager.GetUint64("map.u_int_64")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.UInt64, mapUInt64)

	mapFloat64, err := manager.GetFloat64("map.float_64")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.Float64, mapFloat64)

	mapTime, err := manager.GetTime("map.time")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.Time.UnixNano(), mapTime.UnixNano())

	mapDuration, err := manager.GetDuration("map.duration")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.Duration, mapDuration)

	mapIntSlice, err := manager.GetIntSlice("map.int_slice")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.IntSlice, mapIntSlice)

	mapStringSlice, err := manager.GetStringSlice("map.string_slice")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.StringSlice, mapStringSlice)

	mapStringMapString, err := manager.GetStringMapString("map.string_map_string")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.StringMapString, mapStringMapString)

	mapStringMapStringSlice, err := manager.GetStringMapStringSlice("map.string_map_string_slice")
	assert.Nil(t, err)
	assert.Equal(t, config.Map.StringMapStringSlice, mapStringMapStringSlice)
	...
	assert.Nil(t, err)
	expectedConfData, err := json.Marshal(&config)
	assert.Nil(t, err)
